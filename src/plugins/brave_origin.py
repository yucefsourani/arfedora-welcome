...
class Plugin(BasePlugin):
    ...
    def __init__(self,parent,threads):
        BasePlugin.__init__(self,parent=parent,
                            ...
                            runningmsg="Running...",
                            loadingmsg="Loading...",
                            ifinstallfailmsg="Install Brave Origin Browser Failed",
                            ifremovefailmsg="Remove Brave Browser Origin  Failed",
                            parallel_install=False)


    def check(self):
        return not os.path.isfile("/usr/bin/brave-origin-stable")
        
    def install(self):
        if os.path.isfile("/etc/yum.repos.d/brave-browser.repo"):
            commands = ["dnf install brave-origin -y --best"]
        else:
            commands = ["echo -e '[brave-browser]\nname=Brave Browser\nbaseurl=https://brave-browser-rpm-release.s3.brave.com/$basearch\nenabled=1' > /etc/yum.repos.d/brave-browser.repo",
            "rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc",
            "dnf install brave-origin -y --best"]
    ...
    def remove(self):
        if subprocess.call("pkexec rpm --nodeps -e brave-origin",shell=True)==0:
            return True
        return False
