...
class Plugin(BasePlugin):
    ...
    def check(self):
        return not os.path.isfile("/usr/bin/gimp")
        
    def install(self):
        if subprocess.call("pkexec dnf install gimp -y --best",shell=True)==0:
            return True
        return False
        
    def remove(self):
        if subprocess.call("pkexec rpm --nodeps -e gimp",shell=True)==0:
            return True
        return False
